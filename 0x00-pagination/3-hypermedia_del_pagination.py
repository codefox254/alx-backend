#!/usr/bin/env python3
"""Task 3: Deletion-resilient hypermedia pagination implementation.
"""
...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """Retrieve hypermedia pagination details for a specific index.

        Args:
            index (int): The index to start pagination from.
            page_size (int): The number of items per page.

        Returns:
            Dict: A dictionary containing pagination details including current index, page size, data, and the next index.
        """
        data = self.indexed_dataset()
        assert index is not None and index >= 0 and index <= max(data.keys()), "Invalid index value."
        page_data = []
        data_count = 0
        next_index = None
